...
func Listen(laddr string) (net.Listener, error) { return ListenWithOptions(0,laddr, 0, 0) }
...
func ListenWithOptions(fd int ,laddr string,  dataShards, parityShards int) (*Listener, error) {
	...
	return serveConn(fd, dataShards, parityShards, nil, true)
}
...
func serveConn( fd int ,dataShards, parityShards int, conn net.PacketConn, ownConn bool) (*Listener, error) {
	l := new(Listener)
	l.conn = conn
	l.ownConn = ownConn
...
func NewConn3(convid uint32, raddr net.Addr, dataShards, parityShards int, conn net.PacketConn) (*UDPSession, error) {
	return NewUDPSession(convid, dataShards, parityShards, nil, conn, false, raddr), nil
}

// NewConn2 establishes a session and talks KCP protocol over a packet connection.
func NewConn2(raddr net.Addr, dataShards, parityShards int, conn net.PacketConn) (*UDPSession, error) {
	var convid uint32
	binary.Read(rand.Reader, binary.LittleEndian, &convid)
	return NewConn3(convid, raddr, dataShards, parityShards, conn)
}
...
func NewConn(raddr string, dataShards, parityShards int, conn net.PacketConn) (*UDPSession, error) {
	...
	return NewConn2(udpaddr, dataShards, parityShards, conn)
}
